import { abi as convFactoryAbi } from "../../../artifacts/src/autoConverter/AutoConverterFactory.sol/AutoConverterFactory.json";
...
export async function getConverterRelayInfos(
  ...
  let compounderFactory: Contract = new Contract("0xd4C6eDDBE963aFA2D7b1562d0F2F3F9462E6525b", compFactoryAbi, provider);
...
export async function getConverterTxData(relayInfos: RelayInfo[]): Promise<TxData[]> {
  let txData: TxData[] = [];
  let destinationTokens: string[] = await Promise.all(relayInfos.map((info) => info.contract.token()));
  ...
  relayInfos.forEach((relayInfo: RelayInfo, i: number) => {
    ...
    let calls: string[] = relayInfo.tokens.map((token) => {
      if(token.address == destinationToken) // TODO: Finish excluding destination token
    ...
    txData = txData.concat(calls.map((call) => ({to: relay.address, data: call} as TxData)));
  });
  return txData;
}
